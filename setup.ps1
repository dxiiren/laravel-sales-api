...
# Sales Management System Bootstrap Setup
#
...
Write-Host ""
Write-Host "Sales Management System Bootstrap Setup" -ForegroundColor Cyan
...
# PHP 8.3 is linked against a recent MSVC runtime. If SYSTEM32\VCRUNTIME140.dll
...
$phpDir = Join-Path $env:LOCALAPPDATA "Programs\php-8.3"
$phpExe = Join-Path $phpDir "php.exe"

Refresh-Path
if (Test-Path $phpExe) {
    $phpVer = & $phpExe --version 2>&1 | Select-Object -First 1
    Write-Host "[OK] PHP 8.3 already installed: $phpVer" -ForegroundColor Green
} else {
    $phpZipUrl = "https://windows.php.net/downloads/releases/latest/php-8.3-Win32-vs16-x64-latest.zip"
    $zipPath = Join-Path $env:TEMP "php-8.3-latest.zip"
    Write-Host "[INSTALL] Downloading PHP 8.3 (latest patch) from php.net..." -ForegroundColor Yellow
    try {
        ...
        if (Test-Path $phpExe) {
            $phpVer = & $phpExe --version 2>&1 | Select-Object -First 1
            Write-Host "[OK] PHP 8.3 installed: $phpVer" -ForegroundColor Green
        } else {
...
Write-Host "  3. Start the dev server on http://127.0.0.1:8111 :" -ForegroundColor Gray
