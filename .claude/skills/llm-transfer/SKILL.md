...
situation: what the project is (a Laravel 10 sales-management API: soft-deleted sales,
queued+broadcast InvoiceCreated events logged to invoice.log, REST + Lighthouse GraphQL
daily-sales totals over the Biztory dataset), the domain, and WHY this task matters.
...
## 6. Constraints & Rules
- Tech stack: {Laravel 10 / PHP 8.3 / Lighthouse 6 GraphQL / MySQL schema (biztory.sql) with
  sqlite for local framework tables / PHPUnit 10, versions}
...
Target: _"Hand off a wrong daily-sales total to an external LLM for a fix."_

````text
# MASTER PROMPT - Fix daily-sale total ignoring the payment_status filter

## 1. Role
You are a senior Laravel engineer who writes precise, minimal fixes.

## 2. Mission
Make POST /api/daily-sale honor payment_status=0 (unpaid) instead of silently dropping the
filter, and explain the root cause in one paragraph.

## 3. Background & Context
Laravel 10 sales-management API over an invoicing dataset. POST /api/daily-sale returns the
total of `sales.total` between start_date and end_date, optionally filtered by
payment_status and payee_id. Filtering by payment_status=0 returns the UNFILTERED total.
No repo access - everything you need is below.

## 4. Current State
- Done: date-range totals and payee_id filtering work.
- Tried: confirmed the request validates and payment_status=0 reaches the controller.
- Failing: payment_status=0 produces the same total as no filter at all.

## 5. Relevant Artifacts
`app/Http/Controllers/CountDailySalesController.php`
```
...
## 6. Constraints & Rules
- Laravel 10 / Eloquent. Keep the whereBetween date filter and payee_id handling untouched.
- The GraphQL twin (app/GraphQL/Mutations/DailyTotalSales.php) uses isset() - keep both
  endpoints consistent.

## 7. Your Task
Return the corrected filter condition and a one-paragraph root-cause explanation.

## 8. Output Format & Success Criteria
- Output: the fixed buildQuery() excerpt only + the explanation.
- Done: payment_status=0 filters to unpaid sales; omitting it still returns the full total.

## 9. Open Questions & Assumptions
- Assume payment_status is an integer column where 0 = unpaid, 1 = paid.

## Reminder
Return the fixed filter + a one-paragraph root cause. Keep date/payee filtering untouched.
````
