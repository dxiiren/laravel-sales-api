...
5. **Review in plan mode**, then on approval (create the folder if missing):
   - Write `.claude/workspace/reports/transfers/claude/{YYYY-MM-DD}-{topic}.md`, where
     `{topic}` names **what the session did** (e.g. `add-delivery-status-filter`), **not** this
     skill's name.
...
3. **Re-ground before acting** (do NOT start new work first): read the brief's pointers + the
   ...
   and GET http://127.0.0.1:8111/ + POST /api/daily-sale) to catch drift or undocumented breakage. Building a
...
- Files: `app/Http/Controllers/CountDailySalesController.php:20-35`, `app/Models/Sale.php`
...
- Dev server on :8111 was {up/down} at close - re-check (`just start` / `just stop`)
