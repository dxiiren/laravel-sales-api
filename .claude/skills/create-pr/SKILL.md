...
- If there are multiple commits, synthesize one Conventional title that summarizes
  the change, e.g. `feat(sales): add delivery-status filter to daily-sale endpoint` or
  `fix(graphql): validate payee_id against users on DailyTotalSales`. Pick the type/scope
  from the dominant change, not just the first commit.
...
- <bullet per meaningful change — controller/model/event/GraphQL schema touched>
- <...>

## Testing

- <how it was verified: `just test` result, manual check on http://127.0.0.1:8111, etc.>
```
...
| `/pre-pr-review` | Optional self-review (Laravel/Eloquent/API checklist) before opening the PR   |
