...
For a scoped or careful commit ("commit only this", "commit the sales part"), ignore this
...
| `app/Http/Controllers/`, `app/Http/Requests/`, `routes/`   | `sales` / `api`    |
| `app/GraphQL/`, `graphql/*.graphql`, `config/lighthouse.php` | `graphql`        |
| `app/Events/`, `app/Listeners/`, `app/Providers/EventServiceProvider.php` | `events` |
| `database/factories/`, `database/seeders/`, `biztory.sql`  | `db`               |
...
scope: sales, api, graphql, events, models, db, config, test, build, docs, skills
```
