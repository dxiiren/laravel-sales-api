...
- {Step to bring the world up + how to check it's up + what to do if it's down. e.g. `just
  bootstrap` if `vendor/` is missing, the dev server (`just start` on :8111), a sanity gate
...
A finite, attended-ish goal - close the known sqlite test-schema gap so the suite runs locally:

```markdown
# GOAL - Make the sales test suite runnable on the local sqlite database
...
Add a `sales` (+ minimal `users`) migration mirroring the columns the code actually touches
(`Sale::$fillable` + `SaleFactory`), then point `phpunit.xml` at sqlite `:memory:` with
`RefreshDatabase`, so the four `tests/Unit/*` tests pass without the MySQL `biztory.sql`
import. Success = `just test` green on a machine with no MySQL.

## Locked decisions (do not re-litigate)

- Test behavior, not implementation - assert response content + database state, not internals.
- Use `SaleFactory` for fixtures - never hand-inserted rows.
- NEW migration files only - never edit the four committed framework migrations.

## Environment bootstrap

- `just bootstrap` if `vendor/` is missing. No dev server needed - the tests boot the app
  themselves. Sanity gate: `just test` currently fails with `no such table: sales` - that
  exact failure is the baseline you are fixing.

## Hard prohibitions / guardrails

- NEVER git commit/push/stage - leave changes in the working tree for review.
- No GitHub PR/issue posts.
- Do NOT edit `biztory.sql`, existing migrations, or `config/database.php`.

## What counts as a REAL blocker

- Real: a test asserts a column that exists only in `biztory.sql` with a MySQL-only type
  that sqlite cannot represent - cite the `file:line` and the dump's DDL.
- Not a blocker: "the sales table has many columns" - migrate the ones the code touches.
...
Global: **G1** all four test classes pass. **G2** `just test` fully green on sqlite.

## Work List

| #   | Item                          | Status |
| --- | ----------------------------- | ------ |
| 1   | sales/users migration         | `TODO` |
| 2   | phpunit.xml sqlite :memory:   | `TODO` |
| 3   | StoreSaleControllerTest       | `TODO` |
| 4   | CountDailySalesControllerTest | `TODO` |
| 5   | FactoryStoreSalesTest         | `TODO` |
| 6   | GraphQLDailyTest              | `TODO` |
